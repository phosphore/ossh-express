...
exports.init = function(req, res){
  if (req.isAuthenticated()) {
    ...
    res.render('join/researcher/index', {
      oauthMessage: ''
    });
  }
};
...
exports.signup = function(req, res){
  ...
  workflow.on('validate', function() {
    ...
    else if (!/^[a-zA-Z0-9\-\_]+$/.test(req.body.userinfo.username)) 
    ...
    if (!req.body.userinfo.password) {
      ...
      workflow.outcome.errfor.password = 'password must be over 8 characters';
    }

    if (!req.body.researcherinfo.bio) {
      workflow.outcome.errfor.bio = 'required';
    }
    if (!req.body.researcherinfo.language) {
      workflow.outcome.errfor.language = 'required';
    }
    if (!validgithubAvatar.test(req.body.researcherinfo.avatar) && !validbitbucketAvatar.test(req.body.researcherinfo.avatar)) {
        workflow.outcome.errfor.avatar = 'required';
    }
  ...
  workflow.on('createAccount', function() {
    var fieldsToSet = {
      ...
      background: {
        website: req.body.researcherinfo.website,
        url: req.body.researcherinfo.url,
        bio: req.body.researcherinfo.bio,
        language: req.body.researcherinfo.language,
        certifications: req.body.researcherinfo.certifications,
        iscnumber: req.body.researcherinfo.iscnumber
      },
      search: [
    ...
    req.app.db.models.ResearcherAccount.create(fieldsToSet, function(err, account) {
      if (err) {
        return workflow.emit('exception', err);
      }

      //update user with account
      workflow.user.roles.researcheraccount = account._id;
  ...
  workflow.on('saveUserAvatar', function() {
      ...
      download(req.body.researcherinfo.avatar, "./public/avatars/"+workflow.user._id, function(){}, false);
